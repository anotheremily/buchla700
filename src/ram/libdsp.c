  ...
  for (i = 0; i < FCMAX; i++)
    {
      ...
      sprintf (fcebuf, "000 Empty-%02.2d ??? %-37.37s%c%c",
	       i, "1234567890123456789012345678901234567", A_CR, A_LF);
     ...
     long bytes;
{
  ...
  rclusts = (bytes / _thebpb->clsizb) + ((bytes & clmask) ? 1 : 0);
...
*/
  ...
  if (rc != FCMAX)
      ...
      if (rc)
	{
	  ...
	  sprintf (ldmsg2, "  errno = %d, fesize=%d", errno, fesize);
...
*/
  ...
  if (rc != FCMAX)
      ...
      if (rc)
	{
	  ...
	  ldermsg ("Can't write catalog", ldmsg1, ldmsg2, LD_EMCF, LD_EMCB);
     ...
     unsigned slot, kind;
{
  ...
  sprintf (thename, "M7SLOT%02.2u.%-3.3s", slot, ftypes[kind - 1][0]);
  ...
  if (ckstor ())
    {
      ...
      ldermsg ("No file type selected", ld_em1, ld_em2, LD_EMCF, LD_EMCB);
     ...
     register short n;
{
  ...
  tsplot4 (librob, 64, ldbox[n][4], ldbox[n][6], ldbox[n][7], ldbxlb0[n], 14);
...
*/
  switch (n)
    ...
    case 3:			/* current file name */
      ...
      tsplot4 (librob, 64, ldbox[n][4], ldbox[n][6], ldbox[n][7], ldfile, 14);
      return;

    case 5:			/* current comment field */

      tsplot4 (librob, 64, ldbox[n][4], ldbox[n][6], ldbox[n][7], ldcmnt, 14);
      return;
...
*/
	...
	cy = ldbox[n][4];
      ...
      tsplot4 (librob, 64, cy, ldbox[n][6], ldbox[n][7], "Content", 14);
