...
*/
  switch (n)
    ...
    case 14:			/* point number */
      ...
      tsplot4 (instob, 64, idbox[n][4], idbox[n][6] + 1, idbox[n][7], bfs,
	       14);
...
*/
	...
	case AC_KYUP:
	  ...
	  sprintf (bfs, "GoTo %02d if key %c", pnt, SP_UPA);
	  s1 = bfs;
	  break;

	case AC_KYDN:

	  sprintf (bfs, "GoTo %02d if key %c", pnt, SP_DNA);
	  s1 = bfs;
	  break;
	...
	default:
      ...
      tsplot4 (instob, 64, idbox[n][4], idbox[n][6] + 1, idbox[n][7], s1, 14);
