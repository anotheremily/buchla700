...
msm ()
{
  ...
  for (miport = 0; miport < NMPORTS; miport++)
    {				/* for each port ... */

      while (BIOS (B_RDAV, mpmap[miport]))
	{			/* anything there ? */
		...
		{		/* data */
		  ...
		  switch (mistat)
		      ...
		      if (mdb2)
			{
			  ...
			  trgtab[t1] |=
			    mpsust[(miport << 4) + michan] | M_KSTATE;
