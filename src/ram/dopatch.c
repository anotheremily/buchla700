     ...
     short slot, voice, wsn;
{
  ...
  if (slot)
    {				/* waveshape slot B */
      ...
      memcpyw (vbufs[voice].idhwvbf, &wslib[wsn], NUMHARM + (2 * NUMWPNT));
    ...
    {				/* waveshape slot A */
      ...
      memcpyw (vbufs[voice].idhwvaf, &wslib[wsn], NUMHARM + (2 * NUMWPNT));
     ...
     register struct patch *pp;
{
  ...
  if (ST_VGT & sat)
    ...
    {				/* port, chan, trig sub-address types */
