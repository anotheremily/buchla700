			  ...
			  if (debugsw AND debugxp)
			    {
			      ...
			      printf
				("ptxkey():  DELETING DEF  trg=$%04.4X  stm=$%04.4X\n",
				 i, stm);

			      printf
				("ptxkey():  DELETING DEF  pp=%d  cp=%d  sp=%d\n",
				 pp, cp, sp);
			    }
#endif
     ...
     register unsigned short val;
{
  ...
  switch (port)
    {				/* use port to decode possible types */

    case 0:			/* MIDI-1 - Keys */
    case 1:			/* MIDI-2 - Keys */

      sprintf (buf, "%s %03u %u %02u",
	       (val & 0x8000) ? "Rel" : "Key", 1 + stim, 1 + port, 1 + chan);
...
*/
    ...
    case 2:			/* Local Keys */
      ...
      switch (chan)
	{			/* use channel to further decode */

	case 0:		/* Keys - Local Keyboard */

	  sprintf (buf, "%s %03u L   ",
		   (val & 0x8000) ? "Rel" : "Key", 1 + stim);
...
pafpu2 (bf, ad, os, gt, gn, pp)
     ...
     register struct patch *pp;
{
  sprintf (bf, "%s %d %s %c%c %s", ad, os, stdata[pp->padat1], gt, gn,
	   dfpdat (pp));
}
  ...
  switch (PE_SPEC & pp->paspec)
    ...
    case PA_KEY:		/* key action */
      ...
      sprintf (buf, "K %03u %u %02u    %s",
	       1 + stim, 1 + port, 1 + chan, trdata[pp->padat2 & 0x0003]);
    ...
    case PA_TRG:
      ...
      sprintf (buf, "Trigger %02u    %s",
	       1 + stim, ssdata[pp->padat2 & 0x0001]);
    ...
    case PA_PLS:
      ...
      sprintf (buf, "Pulse %u       %s",
	       1 + stim, trdata[pp->padat2 & 0x0003]);
...
*/
    ...
    case PA_SLIN:
      ...
      sprintf (buf, "Seq %02u Lin    %03u", 1 + pp->pasuba, pp->padat1);
    ...
    case PA_SCTL:
      ...
      sprintf (buf, "Seq %02u Ctl    %s",
	       1 + pp->pasuba, ssdata[pp->padat2 & 0x0001]);

      break;

    case PA_TUNE:

      sprintf (buf, "Tune Table    %u", pp->padat1);
...
*/
    ...
    case PA_INST:
      ...
      sprintf (buf, "Instrument %c%c %02u", vgt, vgn, pp->padat1);
...
*/
	...
	case PSO_INT:		/* interval */
	  ...
	  sprintf (buf, "Osc %u Int  %c%c %c%04u", osc, vgt, vgn, sgn, val);
	...
	case PSO_RAT:		/* ratio */
	  ...
	  sprintf (buf, "Osc %u Rat  %c%c %s", osc, vgt, vgn, ebuf);
	...
	case PSO_FRQ:		/* frequency */
	  ...
	  sprintf (buf, "Osc %u Frq  %c%c %02u.%u", osc, vgt, vgn, vp1, vp2);
...
*/
	case PSO_PCH:		/* pitch */
	  ...
	  sprintf (buf, "Osc %u Pch  %c%c %s", osc, vgt, vgn, ebuf);
...
*/
    ...
    dowaves:
      ...
      sprintf (buf, "Waveshape%c %c%c %02u", sgn, vgt, vgn, pp->padat1);

      break;

    case PA_CNFG:

      sprintf (buf, "Configure  %c%c %02u", vgt, vgn, pp->padat1);
