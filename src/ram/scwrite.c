...
*/
	...
	case EV_ANRS:		/* analog resolution */
	  ...
	  sc += ep->e_type + ep->e_time + ep->e_data1 + ep->e_data2;

	  break;

	case EV_TRNS:		/* transposition */

	  sc += ep->e_type + ep->e_time + ep->e_data1 + (short) ep->e_lft;
...
scsizer ()
{
  ...
  for (i = 0; i < N_SCORES; i++)
      ...
      if (scores[i] NE E_NULL)
	{
	  ...
	  nb += scsize (i) + 16L + (long) (N_SECTS * sizeof (struct s_time));
     ...
     register FILE *fp;
{
  ...
  ldwmsg ("Busy -- Please stand by", (char *) 0L, scid, LCFBX10, LCBBX10);
