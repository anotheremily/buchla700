/*
   =============================================================================
	reversi2.c -- REVERSI-II Desk Accessory.
	Version 6 -- 1988-09-21 -- Mods by D.N. Lynx Crowe

	Written by Christophe Bonnet.  5/08/86

	Define TWIDDLE to slow down the computer...
   =============================================================================
*/
...
int xdesk, ydesk, hdesk, wdesk, hchar, wchar;
int hbox, wbox, xold, yold, hold, wold;
int xwork, ywork, hwork, wwork, hidden, fulled, handed, screen_rez;
int msgbuff[8], keycode, mx, my, butdown, pstatus, ret, ret1, ret2, ret3,
  ret4;
int contrl[12], intin[128], ptsin[128], intout[128], ptsout[128];
int work_in[11], work_out[57], pxyarray[10];

typedef struct grect
{				/*  this structure for clipping and rectangle   */
...
/* These are used in the main part of the program: game variables ***********/
...
int score[4], phase, adv, pass, v, w, valide, value, end = FALSE;
...
open_window ()
{
  ...
  wind_set (wi_handle, WF_NAME, " REVERSI-II ", 0, 0);
  wind_set (wi_handle, WF_INFO, " ", 0, 0);

  wind_open (wi_handle, xdesk + wdesk / 2 - (w_width / 2),
	     ydesk + hdesk / 2 - (w_heigth / 2), w_width, w_heigth);
...
main ()
{
  ...
  menu_id = menu_register (gl_apid, "  Reversi-II");
  ...
  while (TRUE)
    {
      ...
      event = evnt_multi (MU_MESAG | MU_BUTTON | MU_KEYBD | MU_M1 | MU_M2,
			  ...
			  msgbuff, 0, 0, &mx, &my, &ret, &ret, &keycode,
			  &ret);
...
*/
	  case WM_CLOSED:
		...
		graf_shrinkbox (xwork + wwork / 2, ywork + hwork / 2, 0, 0,
...
*/
      ...
      if ((event & MU_KEYBD) && (top_window == wi_handle))
	{
	  ...
	  pass = 1;		/* if the player pressed a key  */
      ...
      for (j = 1; j < 9; j++)
	{
	  ...
	  if (c_grid[i][j] > 1)
		...
		vsf_color (handle, 0);
	      ...
	      v_ellipse (handle,
			 xwork + (i * c_width) - x_border + x_offset,
			 ywork + (j * c_heigth) - y_border + y_offset,
			 x_rad, y_rad);
	    }
	}
    }
...
*/
  ...
  v_ellipse (handle, xwork + wwork - x_rad - 3, ywork + dial_a, x_rad + 3,
	     y_rad + 3);
  v_ellipse (handle, xwork + wwork - x_rad - 3, ywork + dial_b, x_rad + 3,
	     y_rad + 3);
  ...
  wind_set (wi_handle, WF_INFO, "Your move...", 0, 0);
...
player ()
{
  ...
  /* transform mouse coordinates into grid coordinates */
      ...
      for (j = 1; j < 9; j++)
	{
	  ...
	  if (c_grid[i][j] == 1)
	      ...
	      if (flip > 0)
		{
		  ...
		  if (score[2] + score[3] < 34)
		    flip = (-flip);

		  if (score[2] + score[3] > 56)
		    flip = flip * 6;
...
display ()
{
  ...
  rectangle (xwork + wwork - tab, ywork + score_a - hchar, wbox * 2, hbox,
	     BLACK);
  rectangle (xwork + wwork - tab, ywork + score_b - hchar, wbox * 2, hbox,
	     BLACK);
     ...
     int a, b;
{
  ...
  v_ellipse (handle, xwork + (a * c_width) - x_border + x_offset,
	     ywork + (b * c_heigth) - y_border + y_offset, x_rad, y_rad);
...
stop ()
{
  ...
  tens = pgames / 10;
  units = pgames - (tens * 10);
  string[0] = tens + 48;
  string[1] = units + 48;
  string[2] = ':';

  tens = mgames / 10;
  units = mgames - (tens * 10);
  string[3] = tens + 48;
  string[4] = units + 48;
  string[5] = '\0';
...
/* allocate values to drawing variables, depending on screen resolution ****/
