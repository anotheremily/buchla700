...
*/
	...
	FOREVER {		/* cycle through args present */
...
char *path;
{
	if (clsize EQ 0) {		/* if size not already set */

		if ((clsize = getcl(path)) EQ 0) {	/* get cluster size for drive */
...
FILE *fp;
{
	...
	if (tsc AND ++lc GE SCRSIZ) {	/* pause if output is to console screen */
