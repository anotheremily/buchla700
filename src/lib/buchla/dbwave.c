/*
   =============================================================================
	dbwave.c -- Harmonic table file generator for MIDAS-VII
...
#define	PGMNAME		"dbwave"
#define	VERSION		"Version 16 -- 1988-09-08"
...
#define	N		127	/* number of points scanned */
#define	PER		16	/* values per line */

#define	CENTER		(WIDTH / 2)	/* center of the table */
...
main ()
{
  ...
  fprintf (fp, "/*\n%s\n", bar);
  fprintf (fp, "\tCalculated by dbwave.c -- %s\n", VERSION);
...
*/
      for (; j < (L + 1); j++)
	{
	  ...
	  a = cos ((double) i * y);
	  c = ((double) RANGE * a) + 0.5;

	  sprintf (intc, "%5d", (int) c);
