...
main()
{
	...
	if (NULL EQ (fp = fopen(FILEOUT, "wa"))) {	/* open output file */
...
#if DEBUGIT
	for (f = (double)0.0; f < (double)16.0; f += ((double)0.05 * (double)NOTIFY1)) {
#else
...
#if DEBUGIT
	for (p = (double)160.0; p < HILIMIT; p += ((double)0.5 * (double)NOTIFY2)) {
#else
...
*/
	...
	for (i = 0; i < 32768; i++) {	/* split into high and low bytes */
